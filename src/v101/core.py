...
import winshell
import shutil
from tkinter import messagebox  # 仅用于 create_uninstall_script 的临时错误处理
...
def register_application(exe_path, app_name, app_version, publisher, force_register=False):
    ...
    try:
        ...
        if not uninstaller_path: return False, "创建卸载脚本失败。"
        uninstall_key_path = r"Software\Microsoft\Windows\CurrentVersion\Uninstall"
                ...
                for i in range(winreg.QueryInfoKey(key)[0]):
                    ...
                    try:
                        with winreg.OpenKey(key, app_key_name) as app_key:
                            ...
                            if "GreenUninstall.bat" not in uninstall_string: continue
                            try: display_name, _ = winreg.QueryValueEx(app_key, "DisplayName")
                            except FileNotFoundError: display_name = app_key_name
                            try: publisher, _ = winreg.QueryValueEx(app_key, "Publisher")
                            except FileNotFoundError: publisher = "N/A"
                            try: version, _ = winreg.QueryValueEx(app_key, "DisplayVersion")
                            except FileNotFoundError: version = "N/A"
                            try:
                                size_kb, _ = winreg.QueryValueEx(app_key, "EstimatedSize")
                                size = format_size(size_kb * 1024)
                            except FileNotFoundError: size = "N/A"
                            app_data.append({"key_name": app_key_name, "display_name": display_name, "publisher": publisher, "version": version, "size": size})

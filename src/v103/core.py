...
def run_as_admin():
    ...
    if sys.platform == 'win32':
        try:
            ...
            # 如果启动失败（例如用户取消UAC），我们不需要做任何事，
            # 因为 finally 块会处理退出。
            # 打印错误信息有助于调试。
            print(f"UAC请求未能成功执行 (可能是用户取消): {e}", file=sys.stderr)
        finally:
            # 无论 try 块是否成功，此块都将被执行。
            # 这是确保旧进程被终结的最可靠方法。
            # print(">>> FINALLY BLOCK REACHED! EXITING NOW! <<<") 这是在处理下一条命令导致的无法退出的BUG时调试用的金丝雀信息。
            os._exit(0)  # 这里曾经是 sys.exit(0)，但在大多数情况下可能无效，推测可能是被tkinter的主循环阻止了正常退出。暂时用 os._exit(0) 实现强制退出。

    return False # 在非windows平台，此函数无效
